...
{
    public partial class InsertOperationForm : Form
    {
        ...
        private async void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (OptionalIDs.Any())
                {
                    foreach (TextBox textBox in textBoxes)
                    {
                        ...
                        {
                            if (OptionalIDs.Contains(textBoxes.IndexOf(textBox)+1))
                            {
            ...
            SqlCommand command = new SqlCommand($"INSERT INTO [{table}] ({dataFirst})VALUES({dataSecond})", connection);
